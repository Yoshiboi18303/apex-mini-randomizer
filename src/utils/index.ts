...
const legendTypes: ReadOnlyArray<string> = [
  "Assault",
  "Skirmisher",
  "Recon",
  "Support",
...
export const ammoTypes: ReadOnlyArray<string> = [
  "Light",
  "Heavy",
  "Energy",
  "Sniper",
...
function addWeapon(
  weaponName: string,
  ammoType: AmmoType,
  isCarePackageWeapon: boolean = false,
  addInfoURL: boolean = true
): Weapon {
...
const allMaps = new Map<number, ApexMap>([
  [
    0,
    addMap("Kings Canyon", images.maps["King's Canyon"], [
  ...
  [
    1,
    addMap("World's Edge", images.maps["World's Edge"], [
  ...
  [
    2,
    addMap("Olympus", images.maps["Olympus"], [
  ...
  [
    3,
    addMap("Storm Point", images.maps["Storm Point"], [
  ...
  [
    4,
    addMap("Broken Moon", images.maps["Broken Moon"], [
...
const presets = new Map<number, Preset>([
  [
    0,
    addPreset("Weapon Based", [
      makeConsequence(
        "Only One Weapon",
        "You can only use one weapon (no swapping) and your fists for the entire match.",
        "high"
      ),
      makeConsequence(
        "No SMGs",
        "You cannot use any SMGs for the entire match.",
        "low"
      ),
      makeConsequence(
        "No ARs",
        "You cannot use any Assault Rifles for the entire match.",
        "low"
      ),
      makeConsequence(
        "Snipers Only",
        "You can only use any weapons that consume Sniper Ammo (which includes the Wingman).",
        "medium"
      ),
      makeConsequence(
        "Minigun Only",
        "You can only use Sheila for the entire match, nothing else (except your tactical).",
        "high"
      ),
      makeConsequence(
        "Melee Madness",
        "Melees only, that's literally it.",
        "high"
      ),
      makeConsequence(
        "Octane's Favorite",
        "You can only use grenades.",
        "medium"
      ),
      makeConsequence(
        "One In The Chamber",
        "You only get to fire one bullet/burst on every weapon you pick up, then you have to drop it and never pick it back up.",
        "high"
      ),
      makeConsequence(
        "No Hate",
        "You can only use your most disliked weapons",
        "medium"
      ),
    ]),
  ],
  [
    1,
    addPreset("Legend Based", [
      makeConsequence(
        "Gotta Go Fast",
        "As Octane, you can't stop using your stim.",
        "low"
      ),
      makeConsequence(
        "No Tactical Ability",
        "Someone hacked your tactical ability and now it doesn't work.",
        "high"
      ),
      makeConsequence(
        "No Ultimate Ability",
        "Someone hacked your ultimate ability and now it doesn't work.",
        "high"
      ),
      makeConsequence(
        "Drone Safety",
        "As Crypto, you have to stay within 10 meters of your drone, if it gets destroyed, you must wait for it to come back before continuing.",
        "medium"
      ),
      makeConsequence(
        "Broken Jump Jets",
        "As Horizon, you cannot make a hard landing (except from landing from the drop ship), if you do make a hard landing, drop a weapon.",
        "high"
      ),
      makeConsequence(
        "Hacked Revive Shield",
        "As Newcastle, you have to drop your knockdown shield before reviving your teammate(s).",
        "medium"
      ),
      makeConsequence(
        "Copycat",
        "As Mirage, you have to copy at least 5 seconds of your sent decoys' actions before it despawns, if you don't, drop all your weapons (does not include your ultimate) (no taking control of the decoy for those 5 seconds).",
        "low"
      ),
    ]),
  ],
  [
    2,
    addPreset("Movement Based", [
      makeConsequence(
        "The Floor Is Lava",
        "You cannot touch any floor at all, if you do, thermite yourself, if you don't have a thermite, drop a weapon.",
        "high"
      ),
      makeConsequence(
        "Overweight",
        "You cannot sprint at all for the entire match, if you do, drop a weapon.",
        "medium"
      ),
      makeConsequence(
        "Parkour Pro",
        "As Pathfinder, you must always be grappling or off the floor, if you touch the floor, drop a weapon.",
        "high"
      ),
    ]),
  ],
  [
    3,
    addPreset("Controller Based", [
      makeConsequence(
        "Opposite Day",
        'Invert your camera and movement controls (set your stick layout to Southpaw), for extra challenge (optional), turn the "Inverted Look" option on.',
        "high"
      ),
      makeConsequence(
        "One Hand Only",
        "You can only use one hand the entire match (you can hold your controller with your second hand if you need to).",
        "medium"
      ),
      makeConsequence(
        "No ADS",
        "You cannot ADS (Aim Down Sights) for one match.",
        "low"
      ),
    ]),
  ],
...
export function setRandomLegend(
  allowedTypes: boolean[],
  set: Setter<Legend | null>
): void {
  if (allowedTypes.length !== 5)
    throw new Error(getErrorMessage(allowedTypes, 5));
 ...
 * @returns Whether the possibility is successful or not (boolean)
 */
export function getPossibilitySuccessful(
  requiredValue: number,
  operator: MathOperator = "greater"
): boolean {
  if (requiredValue < 0 || requiredValue > 1)
    throw new Error("requiredValue has to be equal to or between 0 and/or 1!");

  const randomNumber = Math.random();
  return calculatePossibility(randomNumber, requiredValue, operator);
}

export function setRandomWeaponLoadout(
  allowedTypes: boolean[],
  carePackageWeaponsAllowed: boolean,
  set: Setter<Weapon[] | null>
): void {
  if (allowedTypes.length !== 5)
    throw new Error(getErrorMessage(allowedTypes, 5));
  ...
  if (carePackageWeaponsAllowed)
    for (const weapon of weaponMap.get(5)!) allWeapons.push(weapon);
...
export function getMapArray(
  set: Setter<ApexMap[]> | null = null
): ReadOnlyArray<ApexMap> {
...
export function getReadOnlyPresetArray(
  set: Setter<Preset[]> | null = null
...
export function setLandingPoint(
  selectedMap: ApexMap | number,
  set: Setter<Location | null>,
  locationsOnMapOnly: boolean = false
): void {
  const map =
    typeof selectedMap === "number" ? allMaps.get(selectedMap) : selectedMap;

  if (map) {
    const locations = locationsOnMapOnly
      ? map.locations.filter((location) => location.shownOnMap === true)
      : map.locations;
...
export function getImagesKeys(
  from: keyof Images = "maps",
  set: Setter<string[] | null> | null = null
): string[] | null {
  if (images[from]) {
...
export function setRandomConsequence(
  consequences: Consequence[],
  set: Setter<Consequence | null>,
  consequenceFilter: ConsequenceSeverity | null = null
): void {
  if (consequenceFilter)
    consequences = consequences.filter(
      (consequence) => consequence.severity === consequenceFilter
    );
...
export function getConsequenceDifficulty(
  severity: ConsequenceSeverity
): string {
  switch (severity) {
    case "low":
...
/**
 * Tries to get the value of `key` from `localStorage`, and returns `defaultValue` if it doesn't exist.
 *
 * **NOTE:** This should only be used on **non-string values** (`Array`, `Object`, etc.), otherwise `JSON.parse` will error.
 *
 * @param key The key in localStorage to get from.
 * @param defaultValue The default value to return if `key` doesn't exist in `localStorage`.
 */
